...
from linersock import Host, Server, Client
from linersock.test_helpers import Message, Inbox, Outbox

wrapper_suite = finalexam.Suite("Testing the socket wrappers...")
...
@wrapper_suite.test
def test_isolated_pipes(helper):
...
@wrapper_suite.test
def test_connected_pipes(helper):
...
@wrapper_suite.test
def test_simple_messages(helper):
    host, port = 'localhost', 10236
...
def test_stressful_conditions(count, bytes):
    sender, receiver = linersock.test_helpers.connect()
    ...
    linersock.test_helpers.disconnect(sender, receiver)
...
def test_token_serialization(helper):
    """
    Test the situation where two messages are sent, and the second one can't be 
    unpacked until the first one is processed.  This situation comes up fairly 
    often in the games that this module was written for.  Handling it requires 
    that pipe.receive() returns messages as an iterator, so that each message 
    can be processed in turn.
    """
